...
const requireContextRegex: RegExp =
  /require\.context\(([\w\W]+?)\)($|\)|;|\n|\r\n)/g;
  ...
  return {
    ...
    async transform(code: string, id: string) {
      ...
      requireContextMatches.forEach((requireContextMatch, index) => {
        const { paramsSyntax, tobeReplacedSyntax } = handleRequireContextSyntax(
          requireContextMatch[0],
        );
        const params: string[] = paramsSyntax.split(',');
        const directory: string = params[0] || '';
        const recursive: boolean = !params[1]
          ? DEFAULT_USE_RECURSIVE
          : eval(params[1]);
...
function handleRequireContextSyntax(originalSyntax: string) {
  if (!originalSyntax.startsWith('require.context')) {
    throw new Error(
      `Unexpected syntax met. Syntax does not start with 'require.context'`,
    );
  }
...
function transformRequireContext(
  ...
  const key2FilesMapString = generateKey2FilesMapString(
    key2FilesMap,
    requireContextMapName,
  );
  const contextFunctionString = generateContextFunctionString(
    requireContextFunctionName,
    requireContextMapName,
  );
...
function generateKey2FilesMapString(
  ...
  key2FilesMapString =
    key2FilesMapString.substring(0, key2FilesMapString.length - 2) + '\n};\n';
